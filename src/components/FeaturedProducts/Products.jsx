...
export default function Products() {
  ...
  const getProductId = (product) =>
    product?._id || product?.id || product?.uuid || "";
  ...
  const toggleFavorite = async (product) => {
    ...
    const favorite = wishlistIds.has(productId);
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-10 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl">
            ...
            >
              Curated products from the Koda Store API with a smooth light and
              dark experience.
            </p>
          </div>
        ...
        {loading ? (
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className={`animate-pulse rounded-3xl p-5 transition-all
                ${
                  isDarkMode
              ...
              >
                <div
                  className={`mb-4 h-52 rounded-2xl ${
            ...
            {visibleProducts.map((product) => {
              ...
              return (
                <ProductCard
                  ...
                  isDarkMode={isDarkMode}
                  isFavorite={
                    productId ? wishlistIds.has(productId) : false
                  }
