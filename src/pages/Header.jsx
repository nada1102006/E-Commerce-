  ...
  useEffect(() => {
    ...
    const loadWishlistCount = async () => {
      try {
        ...
        const items = getWishlistItems(response.data);
        if (mounted) setWishlistCount(items.length);
      } catch (error) {
        if (mounted) setWishlistCount(0);
      }
    };
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          >
            <FiHeart />
            {wishlistCount > 0 && (
              <span className="absolute -top-1 -right-1 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-pink-500 px-1.5 text-[10px] font-semibold text-white">
                {wishlistCount}
              </span>
            )}
          </Link>
